document.addEventListener('DOMContentLoaded', () => {
    ...
    const dinoMascot = document.getElementById('dino-mascot');
    ...
    let activeCell = null;
    let score = localStorage.getItem('TermoScore') ? parseInt(localStorage.getItem('TermoScore')) : 0;
    
    // Inicializar o jogo
    function initGame() {
        scoreDisplay.textContent = "Placar: " + score;
        dinoMascot.classList.remove('celebrate');
    ...
    function checkGuess() {
        ...
        if (guess === targetWord) {
    score++;
    localStorage.setItem('TermoScore', score);
    scoreDisplay.textContent = "Placar: " + score;
    
    // Dinossauro celebra!
    dinoMascot.classList.add('celebrate');
    setTimeout(() => {
        dinoMascot.classList.remove('celebrate');
    }, 600);
    ...
    window.resetScore = function() {
        if (confirm('Tem certeza que quer resetar o placar?')) {
            score = 0;
            localStorage.setItem('TermoScore', 0);
            scoreDisplay.textContent = "Placar: 0";
        }
    };
